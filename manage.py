...
def create_post(user_input):
    ...
    if os.path.exists(filepath):
        ...
        print(
            "Post created: ./_posts/"
            + filename
            + "\nImage namespace at: /assets/posts/"
            + _file_name
            + "/"
        )
...
current_page_platform: {2}""".format(
            category[1], category[0], category[0]
        )

        with open(target_path, "w") as write_file:
            write_file.write(
                page_template.replace(replace_anchor, front_matter_fragement)
            )
...
def modify_post(filepath):
    ...
    front_matter = FRONT_MATTER_TEMPLATE.format(
        title=filename.replace("-", " ").capitalize().replace(".md", ""), date=post_date
    )
    ...
    if IF_CONTAINS_IMAGE:
        ...
        md_content = pattern.sub(
            lambda s: s.group().replace("](", "](" + POST_IMAGE_PREFIX), md_content
        )
